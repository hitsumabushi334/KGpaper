...
""",
        ...
        with patch("kgpaper.llm_extractor.genai.Client", return_value=mock_client):
            with patch("kgpaper.llm_extractor.time.sleep"):
                with patch(
                    "kgpaper.llm_extractor.time.time", return_value=0
                ):  # タイムアウトしない
                    from kgpaper.llm_extractor import LLMExtractor
...
""",
        ...
        with patch("kgpaper.llm_extractor.genai.Client", return_value=mock_client):
            with patch("kgpaper.llm_extractor.time.sleep"):
                with patch("kgpaper.llm_extractor.time.time", return_value=5):
                    ...
                    extractor = LLMExtractor(config_path=str(config_file))
                    result = extractor.upload_file(
                        "test.pdf", progress_callback=progress_callback
                    )
    ...
    def test_upload_file_cleanup_on_timeout(self, tmp_path, monkeypatch):
        """アップロードタイムアウト時のクリーンアップ処理のテスト"""
        # configの準備
        config_file = tmp_path / "config.yaml"
        config_file.write_text(
            """
...
""",
        ...
        mock_file_processing.name = "processing-file"
        mock_file_processing.state.name = "PROCESSING"

        mock_client = MagicMock()
        mock_client.files.upload.return_value = mock_file_processing
        mock_client.files.get.return_value = mock_file_processing  # ずっとPROCESSING

        # タイムアウト設定を短くする
        with patch("kgpaper.llm_extractor.genai.Client", return_value=mock_client):
            with patch("kgpaper.llm_extractor.time.sleep"):  # sleepをスキップ
                with patch(
                    "kgpaper.llm_extractor.time.time", side_effect=[0, 1, 2, 15]
                ):  # 開始, 1回目, 2回目, タイムアウト(>10)
                    from kgpaper.llm_extractor import LLMExtractor
                    ...
                    # 削除メソッドが呼ばれたことを確認
                    mock_client.files.delete.assert_called_with(name="processing-file")
