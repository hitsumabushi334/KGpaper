...
with tab1:
    ...
    uploaded_files = st.file_uploader(
        "Upload Paper PDFs", type=["pdf"], accept_multiple_files=True
    )
    ...
    if st.button("Start Extraction", type="primary", disabled=not uploaded_files):
        ...
        for i, file in enumerate(uploaded_files):
            ...
            try:
                # st.statusで詳細な進捗を表示
                with st.status(
                    f"Extracting from {file.name}...", expanded=True
                ) as status:
                    ...
                    def update_progress(retry_count, elapsed):
                        st.write(
                            f"⏳ Processing... (retry {retry_count}, {elapsed:.0f}s elapsed)"
                        )

                    json_ld = extractor.extract_json_ld(
                        tmp_path, document_type=document_type
                    )
